...
type SortKey =
  | "code" | "name" | "sector" | "currency" | "quantity" | "pru"
  | "currentPrice" | "totalCost" | "totalValue" | "latentGainLoss"
  | "latentGainLossPercent" | "stopLoss" | "risk" | "riskPercent";
...
export function CurrentPositions({
  ...
  // Label "il y a X min" recalculé à chaque render (suffisant, pas besoin d'interval)
  ...
  const positionsWithPrices: Position[] = useMemo(() => {
    return positions.map((p) => {
      ...
      const effectivePrice =
        p.manualCurrentPrice !== undefined ? p.manualCurrentPrice
  ...
  const getRiskPercent = (pos: Position) => pos.stopLoss !== undefined && pos.pru > 0 ? ((pos.stopLoss - pos.pru) / pos.pru) * 100 : undefined;
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input type="text" placeholder="Rechercher par code ou nom..." value={searchFilter} onChange={(e) => setSearchFilter(e.target.value)} className="pl-9" />
            </div>
            ...
            {/* Indicateur cours live — affiché seulement si des positions existent */}
            {symbols.length > 0 && (
                    ...
                    <TableRow key={`${position.portfolioCode || ""}-${position.code}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex flex-col items-end gap-0.5">
                          ...
                          />
                          {/* Prix live affiché en petit si pas d'override manuel */}
                          {hasLivePrice && (
                      ...
                      {portfolioCategory === "Trading" && (
                        <>
                          <TableCell className="text-right">
                            <Input
                              type="number" step="0.01" placeholder="SL"
                ...
                <TableRow className="border-t-2 font-bold bg-muted/50">
                  ...
                  {portfolioCategory === "Trading" && (
                    <>
                      ...
                      <TableCell className={`text-right font-bold ${totalRisk >= 0 ? "text-green-600" : "text-red-600"}`}>{formatPercent((totalRisk / Math.max(1, totalPortfolio)) * 100)}</TableCell>
                    </>
                  )}
                  <TableCell></TableCell>
                </TableRow>
